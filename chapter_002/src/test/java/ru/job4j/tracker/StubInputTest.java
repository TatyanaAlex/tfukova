...
public class StubInputTest {
    ...
    public void whenShowAllThenTrackerFindsAll() {
        ...
        assertThat(tracker.findByName("test name"), is("test name"));
    ...
    public void whenDeleteThenTrackerHasDeletedItem() {
        Tracker tracker = new Tracker();
        Item item = tracker.add(new Item("test name", "desc", 5));
        Input input = new StubInput(new String[]{"3", item.getId(), "test name", "desc", "6"});
        new StartUi(input, tracker).init();
        assertThat(tracker.findById(item.getId()), is(equals(null)));
    }
    @Test
    public void whenPutIdThemTrackerFindsById() {
        ...
        new StartUi(input, tracker).init();
        assertThat(tracker.findAll()[0].getId(), is(equals(item.getId())));
    }
    @Test
    public void whenPutNameThemTrackerFindsByName() {
        Tracker tracker = new Tracker();
        Item item = tracker.add(new Item("test name", "desc", 5));
        Input input = new StubInput(new String[]{"5", item.getId(), "test name", "desc", "6"});
        new StartUi(input, tracker).init();
